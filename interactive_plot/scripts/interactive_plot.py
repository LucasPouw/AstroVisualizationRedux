...
class Interactive(ImagingLofar):
    ...
    def html_from_png(self):
        ...
        x_low, y_low = list(float(i)
                            for i in self.wcs.array_index_to_world_values(0, 0))
        x_high, y_high = list(
            float(i) for i in self.wcs.array_index_to_world_values(
        ...
        p = figure(
            ...
            active_drag='pan')
        p.image_url(['interactive_plot/images/main.png'], x=x_low,
                    y=y_high, h=y_high - y_low, w=x_low - x_high)
