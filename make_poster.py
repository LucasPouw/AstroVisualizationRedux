...
if __name__ == '__main__':
    if args.csvfile:
        df = pd.read_csv(args.csvfile)[
            ['RA', 'DEC', 'size_x', 'size_y']].to_numpy()[0:12]
    else:
    ...
    for n, position in enumerate(df):
        if position[3] == position[3]:
            Image.image_cutout(pos=tuple(position[0:2]), size=tuple(
                position[2:4]), image_name=f'cutout_{n}.png', save=True, cmap='jet')
        else:
            Image.image_cutout(pos=tuple(
                position[0:2]), image_name=f'cutout_{n}.png', save=True, cmap='jet')
