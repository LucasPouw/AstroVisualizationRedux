...
if __name__ == '__main__':
    ...
    if args.csvfile:  # go through all objects in csv file
        ...
        start_coord = Movie.wcs.pixel_to_world(
            Movie.image_data.shape[1] / 2,
            Movie.image_data.shape[0] / 2)
        ...
        for n in range(len(df) - 1):  # stack multiple sources
            if n > 0:
                dist = distance([last_RA, last_DEC], [
                                df['RA'].values[n], df['DEC'].values[n]])
                move_to_frames = max(int(4 * dist * Movie.framerate), 2)
            else:
                dist = distance([start_ra, start_dec], [
                                df['RA'].values[n], df['DEC'].values[n]])
                move_to_frames = max(int(4 * dist * Movie.framerate), 2)
            Movie.move_to(
                N_frames=move_to_frames,
                ra=df['RA'].values[n],
                dec=df['DEC'].values[n])
            zoom_frames = max(int(0.1 * Movie.framerate *
                              Movie.imsize / df['imsize'].values[n]), 2)
            Movie.zoom(N_frames=zoom_frames, imsize_out=df['imsize'].values[n])
            if n < len(
                    df) - 1 and df['imsize'].values[n + 1] > df['imsize'].values[n]:
        ...
        move_to_frames = max(int(
            4 * Movie.framerate * distance([start_ra, start_dec], [last_RA, last_DEC])), 2)
        ...
        start_pix_y = int(
            pix_y_size * ((number_of_steps * 2 - 1) / (number_of_steps * 2)))
        ...
        try:
            positions = [(p.ra.degree, p.dec.degree) for p in [Movie.wcs.pixel_to_world(position[0], position[1])
                                                               for position in positions if not isNaN(Movie.image_data[position[0], position[1]])]]
        except BaseException:
            positions = [
                (p.ra.degree, p.dec.degree) for p in [
                    Movie.wcs.pixel_to_world(
                        position[0], position[1]) for position in positions]]

        Movie.imsize = 2 * \
            abs(fits_header['CDELT1'] * fits_header['CRPIX1'] / number_of_steps)
        ...
        for n, position in enumerate(positions):
            if n == 0:
                ...
                dist = distance([position[0], position[1]], [
                                positions[n - 1][0], positions[n - 1][1]])
            move_to_frames = max(int(4 * dist * Movie.framerate), 2)
            Movie.move_to(
                N_frames=move_to_frames,
                ra=position[0],
