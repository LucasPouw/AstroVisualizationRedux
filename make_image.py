...
parser.add_argument(
    ...
    '--image_size',
    type=float,
    help='Image size in degrees (for squared region)')
parser.add_argument(
    '-ia',
    '--interactive',
    type=bool,
    default=True,
    help='Display image in interactive mode')
...
filename = '_'.join(args.fits.split(
    '/')[-1].split('.')[0:-1] + [str(args.right_ascension), str(args.declination)])

Image = ImagingLofar(
    fits_file=args.fits,
    image_directory='cutouts',
    verbose=False)
Image.image_cutout(
    image_name=filename +
    '.png',
